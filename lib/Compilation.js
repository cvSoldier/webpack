...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_addModule(module, callback) {
		// 从 factorizeQueue 到 addModuleQueue
	...
	_processModuleDependencies(module, callback) {
		...
		const onDependenciesSorted = err => {
			...
			for (const item of sortedDependencies) {
				...
				// 递归起来了，以子 dep 继续调用熟悉的 handleModuleCreation，
	...
	handleModuleCreation(
		...
		this.factorizeModule(
			...
			// factoryResult.module 是 NormalModule
			// 在哪里把 (Dependencies) 转化成module的 Q1
			(err, factoryResult) => {
	...
	_handleModuleBuildAndDependencies(originModule, module, recursive, callback) {
		...
		this.buildModule(module, err => {
			...
			// 好像是处理递归依赖的
			// 自信点，去掉好像
			// 这里的 module 有了 dependencies，从哪儿儿来的
			// this.parser.parse出来的
	...
	_factorizeModule(
		...
		// 在这里 Q1
		// factory.create 在lib/NormalModuleFactory.js
		// 处理loader的相关工作，暂时不用管，关注回调，关注主干逻辑
		factory.create(
			{
				contextInfo: {
	...
	seal(callback) {
		...
		// optimizeDependencies这个钩子估计又是各种内部plugin
		...
		// 好像是根据dep把这一波 module 整理到 chunkGraphInit.get(entrypoint)
		// #region
...
Compilation.prototype.factorizeModule = /** @type {{
	...
	function (options, callback) {
		// 最后会找this.factorizeQueue.processor
		// Q2 为啥看逻辑是找 factorizeQueue 的parent的 processor
