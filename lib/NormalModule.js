...
class NormalModule extends Module {
	...
	_doBuild(options, compilation, resolver, fs, hooks, callback) {
		...
		const processResult = (err, result) => {
			...
			// hello? ast在哪儿转的,
			// this.parser.parse的时候有，这里不知道是干啥的
			this._ast =
	...
	build(options, compilation, resolver, fs, callback) {
		...
		// 内部调用 runLoader 先把文件处理为 javascript
		return this._doBuild(options, compilation, resolver, fs, hooks, err => {
			...
			const handleBuildDone = () => {
				// 调用栈深度的底部，在此调用的callback一路向上，
				// 最终进入 compiler 的 this.hooks.make.callAsync() 的回调，开始下一个 hooks。
				// 🎉🎉🎉
				try {
			...
			try {
				const source = this._source.source();
				// 既然有 loader 了，为什么还需要parse？
				// parse的最大作用就是收集模块依赖关系，
				// 比如调试代码中出现的import {is} from 'object-is'或const xxx = require('XXX')的模块引入语句，
				// webpack会记录下这些依赖项，记录在 module.dependencies 数组中。
